#Donovan Rea's connect 4 modification - connect 5 with two AI players 
#Run python file and follow instructions!
...
class GameBoard:
    ...
    def display(self):
        ...
        print('Points player 3: {}' .format(self.points[2]))
    ...
    def horizontal_check(self,row,col,player):
        ...
        while i < 5:
            try:
                ...
                if (self.items[row][col] == player  and self.items[row][col+1] == player  and self.items[row][col+2] == player  and self.items[row][col+3] == player and self.items[row][col+4] == player):
                    result +=1        
    ...
    def vertical_check(self,row,col,player):
        ...
        try:
            #check all surrounding combinations
            if (self.items[row][col] == player  and self.items[row-1][col] == player  and self.items[row-2][col] == player  and self.items[row-3][col] == player and self.items[row-4][col] == player):
                return 1
    ...
    def right_diagonal_check(self,row,col,player):
        ...
        while i < 5:
            try:
                #check all surrounding combinations
                if (self.items[row][col] == player  and self.items[row+1][col+1] == player  and self.items[row+2][col+2] == player  and self.items[row+3][col+3]== player and self.items[row+4][col+4]== player):
                    result +=1
                col -= 1
                row -= 1
            except IndexError:
...
class FourInARow:
    def __init__(self, size):
        ...
        self.SecondAi_opponent = 2
        self.difficulty_confirmation = 1
        self.helper = 0

    #define the new difficulty function which determines who the second AI plays for!
    def difficulty(self):
        print("\t WELCOME TO CONNECT 5!\n")
        print('\tChoose your difficulty!\n\n 1 = Easy   2 = Medium  3 = Difficult\n')
        A = True
        # the user is required to enter a difficulty level - game will not proceed until valid difficulty is chosen
        while A == True:
            try: 
                self.helper = int(input('Enter Difficulty: '))

                if self.helper != 1 or self.helper != 2 or self.helper != 3:
                    raise ValueError
                continue  
            except ValueError:
                print("Input must either 1, 2 or 3!")
                continue
            finally:
                if self.helper == 1 or self.helper == 2 or self.helper == 3:
                    A == False
        ...
        #dependent on the difficulty setting determines how the second AI plays
        if self.helper != 0:

            self.difficulty_confirmation = self.helper

            if self.helper == 2:
                self.SecondAi_opponent = 3
            elif self.helper == 3:
                self.SecondAi_opponent = 1

    def play(self):
        print("")
        if self.board.size > 10:
            print("* " * (self.board.size//2) + "NEW GAME"  + " *" * (self.board.size//2))
        else:
            print("* * NEW GAME * *")
        ...
        while not self.board.game_over():
            ...
            if player_number==0:
                ...
                print("AI 1 chooses column ", column)
            else:
                # Choose move which would minimises the second Ai's opponent's next spot
                (c,maxPoints)=self.board.column_resulting_in_max_points(self.SecondAi_opponent)
                (c_1, maxPoints_1) = self.board.column_resulting_in_max_points(1)

                #if difficulty is easy, the AI will be playing the other AI, the harder the difficulty, the less the AI plays for you
                if (self.SecondAi_opponent ==2) and (self.difficulty_confirmation == 2):
                    self.SecondAi_opponent = 2
                elif (self.SecondAi_opponent ==2) and (self.difficulty_confirmation != 2):
                    self.SecondAi_opponent = 1
                elif (self.SecondAi_opponent ==1) and (self.difficulty_confirmation == 2):
                    self.SecondAi_opponent = 2
                
                #make sure that the 2nd AI doesn't play against the player when difficulty is set to 1!
                if (c == c_1) and (self.difficulty_confirmation == 1):
                    (c_3,maxPoints_3)=self.board.column_resulting_in_max_points(3)
                    column = c_3
                elif maxPoints>0:
                    column=c 
                else:
                    # if no move minimises new points for the other then choose move which maximises the 2nd AI's point
                    (c,maxPoints)=self.board.column_resulting_in_max_points(3)
                    if maxPoints>0:
                        column=c
                    else:
                        # if no move creates new points then choose column as close to middle as possible
                        column = self.board.free_slots_as_close_to_middle_as_possible()[0]
                
                self.board.add(column, 3)
                print("AI 2 chooses column ", column)

            self.board.display()   
            player_number=(player_number+1)%3

        if (self.board.points[0]>self.board.points[1] and self.board.points[0]>self.board.points[2]):
            print("Player 1 (circles) wins!")
        elif (self.board.points[0]<self.board.points[1] and self.board.points[1]>self.board.points[2]):    
            print("Player 2 (crosses) wins!")
        elif (self.board.points[0]<self.board.points[2] and self.board.points[2]>self.board.points[1]):
            print("Player 3 (plusses) wins!")
        else:  
            print("It's a draw!")
